...
export default function FaceSelection() {
      ...
      try {
        ...
        if (currentSession) {
          ...
          processDetectedFaces(
            currentSession.detected_faces,
            currentSession.base_photo_url
          );
        }
      } catch (err) {
  ...
  if (isProcessing || !session) {
    return (
      <ProcessingOverlay title="Finalizing Selections..." icon={Sparkles} />
    );
  }
    ...
    <div className="max-w-md mx-auto px-4 py-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Tap a face on the base photo to choose a better expression.
        </p>
      </div>
          ...
          >
            ...
            {baseFaces.map((face) => (
              <FaceHighlight
                ...
                isReplaced={
                  selectedReplacements[face.face_id]?.face_id !== face.face_id
                }
              />
            ))}
          </motion.div>
